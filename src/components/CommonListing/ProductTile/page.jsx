      ...
      <div  onClick={()=> router.push(`/product/${item._id}`)}>
        ...
        <div className="my-4 mx-auto flex w-10/12 flex-col items-start justify-between">
          <div className="mb-2 flex">
            <p
              className={`mr-3 text-sm font-semibold text-gray-200 ${
            ...
            {item.onSale === "yes" ? (
              <p className="mr-3 text-sm font-semibold text-gray-200">{`-(${item.priceDrop}%)off`}</p>
            ) : null}
          </div>
          <h3 className="md-2 text-gray-200 text-md">{item.name}</h3>
        </div>
      </div>
    );
  }
